...
const Task3 = (props) => {
    ...
    const [state, updateState] = useState({
        ...
        typingMembers: [], /*Chan: typing indicator*/
    ...
    const handleJoinChannel = async (channelUrl) => {
        ...
        updateState({ ...state, typingMembers: [], loading: true }); //Chan: typing indicator
    ...
    const onMessageInputChange = (e) => {
        const { currentlyJoinedChannel, messageToUpdate } = state; // Chan: typing indicator
    ...
    const sendMessage = async () => {
        ...
        if (messageToUpdate) {
            ...
            currentlyJoinedChannel.sendUserMessage(userMessageParams)
                .onSucceeded((message) => {
                    currentlyJoinedChannel.endTyping();   // Chan: typing indicator
    ...
    const onFileInputChange = async (e) => {
        if (e.currentTarget.files && e.currentTarget.files.length > 0) {
            ...
            currentlyJoinedChannel.sendFileMessage(fileMessageParams)
                .onSucceeded((message) => {
                    const updatedMessages = [...messages, message];   // Chan: typing indicator
    ...
    return (
        <>
            ...
            >
                ...
                />
                {state.typingMembers.length > 0 && DisplayTypingIndicator(state.typingMembers) /*Chan: typing indicator*/}
                <MessageInput
    ...
    if (message.url) {
        return (
            <div className={`message  ${messageSentByYou ? 'message-from-you' : ''}`}>
                ...
                <figure>
                    <img src={message.url} />
                    <figcaption>{timestampToTime(message.createdAt)}</figcaption>
                </figure>
            </div>
        );
    }
